...
def transcribe_file_with_word_time_offsets(speech_file):
    ...
    for result in response.results:
        ...
        dict = {"full_text": alternative.transcript}
        ...
        for i, word_info in enumerate(alternative.words):
            ...
            dict[str(i)] = {"word": word, "startTime": start_time.total_seconds(), "endTime": end_time.total_seconds()}

            print(
                f"Word: {word}, start_time: {start_time.total_seconds()}, end_time: {end_time.total_seconds()}"
            )

    with open(os.path.splitext(speech_file)[0] + ".json", "w") as outfile:
